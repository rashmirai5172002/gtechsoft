    ...
    <section className="py-34 bg-white overflow-hidden">
      <Container>
        <div className="text-center mb-25">
          ...
          <p
            className="text-[28px] sm:text-[36px] md:text-[52px] font-normal leading-[100%] text-center dark:text-black"
            style={{ fontFamily: "'Helvetica Neue', sans-serif" }}
          >
            We build partnerships based
            <br /> on trust and responsibility
          </p>
        </div>
      </Container>
      ...
      <div className="overflow-hidden relative w-full">
        ...
        {/* Auto-scroll row */}
        <div className="flex animate-marquee1 gap-2 md:gap-5 w-max">

  {[...testimonials, ...testimonials].map((item, i) => (
    <div
      key={i}
      className="w-[150px] sm:w-[220px] md:w-[318px] h-[240px] sm:h-[320px] md:h-[432px] rounded-[8px] flex-shrink-0 overflow-hidden"
    >

      {item.type === "text" ? (
        <div
          className={`${item.bg} ${item.textColor} h-full p-2 sm:p-4 md:p-6 flex flex-col justify-between`}
        >

          {/* TEXT */}
          <p className="text-[10px] sm:text-sm md:text-lg leading-snug line-clamp-6 md:line-clamp-none">
            {item.text}
          </p>

          {/* NAME + ROLE */}
          <div className="mt-1">
            <h4 className="text-[11px] sm:text-sm md:text-base font-semibold leading-tight">
              {item.name}
            </h4>
            <p className="text-[9px] sm:text-xs md:text-sm opacity-70 leading-tight">
              {item.role}
            </p>
          </div>
